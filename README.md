...
void compress(vector<uint32_t> &h, const vector<uint32_t> &k, const vector<uint32_t> &w) {
    for (int i = 0; i < 64; ++i) {
        uint32_t S1 = rotr(h[4], 6) ^ rotr(h[4], 11) ^ rotr(h[4], 25);
...
vector<uint32_t> sha256(const string &msg) {
    vector<uint32_t> h = {
    ...
    while (padded.size() % 64 != 56) padded.push_back(char(0x00));
    for (int i = 7; i >= 0; --i) padded.push_back(char((bitLen >> (i*8)) & 0xFF));

    for (size_t off = 0; off < padded.size(); off += 64) {
        vector<uint32_t> w(64, 0);
        for (int t = 0; t < 16; ++t) {
            uint32_t b0 = static_cast<unsigned char>(padded[off + t*4 + 0]);
            uint32_t b1 = static_cast<unsigned char>(padded[off + t*4 + 1]);
            uint32_t b2 = static_cast<unsigned char>(padded[off + t*4 + 2]);
            uint32_t b3 = static_cast<unsigned char>(padded[off + t*4 + 3]);
...
string toHex(const vector<uint32_t> &h) {
    stringstream ss;
    ss << hex << setfill('0');
    for (auto v : h) ss << setw(8) << v;
    return ss.str();
}

int main() {
    string book = R"(1] The beginning of the gospel of Jesus Christ, the Son of God.
...
[20] And they went forth and preached everywhere, while the Lord worked with them and confirmed the message by the signs that attended it. Amen.
    ...
    vector<uint32_t> digest = sha256(book);
    cout << "SHA-256: " << toHex(digest) << endl;
    return 0;
}
